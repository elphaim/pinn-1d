...
class StrategicPINNTrainer:
    ...
    Args:
        model: StrategicPINN or StrategicGeneralizedPINN instance
    ...
    def __init__(
        ...
        # Get inverse problem status
        self.is_inverse = model.is_inverse
        self.inverse_params = model.inverse_params
        ...
        self.lambda_bc = 1.0
        self.lambda_ic = 1.0
        self.lambda_ic_t = 1.0  # For wave equation initial velocity
        self.lambda_m = 1.0 if self.is_inverse else 0.0
        ...
        # Track learned parameters (supports multiple params for generalized PINN)
        if self.is_inverse:
            self.history['learned_params'] = {name: [] for name in self.inverse_params}
        ...
        print(f"  Problem type: {'Inverse' if self.is_inverse else 'Forward'}")
        if self.is_inverse:
            print(f"  Learning: {self.inverse_params}")
    ...
    def _record_learned_params(self):
        """Record current values of learned parameters to history."""
        params = self.model.get_learned_params()
        for name, value in params.items():
            self.history['learned_params'][name].append(value)

    def _print_learned_params(self):
        """Print current values of learned parameters."""
        params = self.model.get_learned_params()
        for name, value in params.items():
            # Try to get true value from PDE for comparison
            true_val = self.model.pde.params.get(name, None)
            if true_val is not None:
                print(f"  {name}: {value:.6f} (true: {true_val})")
            else:
                print(f"  {name}: {value:.6f}")

    def _get_final_learned_params(self) -> Dict[str, float]:
        """Get final values of all learned parameters."""
        return self.model.get_learned_params()
    ...
    def compute_loss_gradients(self) -> Dict[str, float]:
        ...
        return {
            ...
            'grad_norm_m': grad_norm_m.item() if self.is_inverse else 0.0
        }
    ...
    def compute_adaptive_weights(self) -> Dict[str, float]:
        ...
        if self.grad_mean_f is None or self.grad_mean_bc is None or self.grad_mean_ic is None or self.grad_mean_m is None:
            ...
            self.grad_mean_m = grads['grad_norm_m'] if self.is_inverse else 0.0
        else:
        ...
        lambda_m_new = mean_grad / (self.grad_mean_m + 1e-8) if self.is_inverse else 0.0
    ...
    def plot_progress(self, save_path: Optional[str] = None):
        ...
        def plot_params(ax):
            ...
            if 'learned_params' in self.history:
                colors = plt.colormaps['tab10'](np.linspace(0, 1, len(self.inverse_params)))
                for i, name in enumerate(self.inverse_params):
                    values = self.history['learned_params'][name]
                    ax.plot(epochs, values, color=colors[i], linewidth=2, label=f'{name}')
                    # Show true value if available
                    true_val = self.model.pde.params.get(name, None)
                    if true_val is not None:
                        ax.axhline(y=true_val, color=colors[i], linestyle='--',
    ...
    def save_checkpoint(self, path: str):
        ...
        final_params = self._get_final_learned_params() if self.is_inverse else None
        ...
        checkpoint = {
            ...
            'inverse_params': self.inverse_params if self.is_inverse else []
        }

        torch.save(checkpoint, path)
        print(f"Checkpoint saved to {path}")

        # Also save history as JSON (with special handling for nested dict)
...
def train_comparison():
    """
    Train Advection with both strong and weak forms for comparison.
    """
    from pdes.heat import HeatEquation1D
    from data.pde_data import PDEData
    from models.pinn_strategy import GeneralizedStrongFormLoss, GeneralizedWeakFormLoss
    ...
    model_strong = StrategicGeneralizedPINN(pde=pde)
    model_strong.set_loss_strategy(GeneralizedStrongFormLoss())
    ...
    model_weak = StrategicGeneralizedPINN(pde=pde)
    model_weak.set_loss_strategy(
        GeneralizedWeakFormLoss(integration_method='gauss_legendre', 
    ...
    u_exact = pde.analytical_solution(x_flat.numpy(), t_flat.numpy())
